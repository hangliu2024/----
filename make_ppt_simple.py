...
def rect(sl, l, t, w, h, fill, line_color=None, lw=0.75):
    shape = sl.shapes.add_shape(1, Inches(l), Inches(t), Inches(w), Inches(h))
    shape.fill.solid()
    shape.fill.fore_color.rgb = R(*fill)
    if line_color:
        shape.line.color.rgb = R(*line_color)
...
cx = 0.5
for title, num, desc in cards_data:
...
for name, stats, pct in progs:
    ...
    tb(sl, stats, px + 0.15, y3 + 0.62, 2.15, 0.2, 7.5, False, SUB)
    px += 2.45
...
for date, title in ms:
    bg_c = ACCENT if "6/7" in date else NAVY
